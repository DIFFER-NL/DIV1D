...
a_i = \Delta^i\frac{\tau_{\mathrm{r}}}{\sqrt{2}}.
\end{equation}

This follows from the fact that the maximum of equation~(\ref{maxwell_boltzmann_elm}) for $n=0$ is set to coincide with some ELM ramp time $\tau_{\mathrm{r}}$, which is an input parameter. Note that the ELM ramp time does not exactly reflect the maximum value for $n=3$, which is shifted to a slightly later time. Consecutive-order terms in $a_i$ differ by a multiplication factor of $\Delta$. Values of $\Delta=1.9$ and $n=3$ (note that the sum starts from zero, so these are four terms) were chosen to eye-ballingly match the long-tail behaviour found from AXUV bolometry on TCV~\cite{veres2009}. They are fixed in the code.

Equation~(\ref{maxwell_boltzmann_elm}) has the useful property that

\begin{equation}
\label{norm_of_maxwell_boltzmann_elm}
\int_0^\infty\frac{\Phi(t)}{\tilde{\Phi}}\mathrm{d}t=1
\end{equation}

which means that the total amount of energy or particles that is put into the system by the ELM is known beforehand. The total number of particles and amount of heat flux are set by a pair of input parameters.

The subroutine can be used to calculate both the heat flux and particle contributions of an ELM. The former is done by directly adding $\Phi(t)$ to the X-point boundary condition for the heat flux. Alternatively, the latter requires calculating the first derivative of equation~(\ref{maxwell_boltzmann_elm}) with respect to time first, owing to a slightly different formulation of the plasma density boundary condition.

Finally, the code also includes a possibility to simulate a series of ELMs rather than a single one, with a repetition frequency that can also be defined on input.
...
\begin{center}
  ...
  \begin{tabular}{|| l  | l ||}
    ...
    {\tt elm\_ramp\_time}   	 & integer: ramp time of ELM (not exact, see section~\ref{section:elms}), in units of delta\_t (default: 0) \\ \hline
