...
! author of code: Egbert Westerhof, DIFFER (c) 2019
   ...
   ! write the HARDCODED tag for backward compatibility in reading the outputs with Matlab! 
   write( 10, * ) 'git tag: v4.0.0'

   ! read non-default input parameters (this also writes the namelists to unit 10)
...
subroutine write_header
   ...
   write( 10, '(3(1PE15.6))' ) ( x(i), gas_puff(i), B_field(i), i=1,Nx )
   return
...
subroutine write_solution( time )
   ...
   write( 10, '(A133)' ) '    X [m]        N [/m^3]       V [m/s]         T [eV]        Nn [/m^3]      Source_n       Source_v       Source_Q     source_neut  '
   write( 10, '(9(1PE15.6))' ) ( x(i), density(i), velocity(i), temperature(i), neutral(i), &                 !!!! the neutral flux should not be multiplied with the B_field !!!!    
   &                                   Source_n(i), Source_v(i), Source_Q(i), source_neutral(i), i=1,Nx )
   ! write the cell boundary properties (fluxes)
   write( 10, '(A76)' )  '   Xcb [m]       Gamma_n    Gamma_mom [Pa]     q_parallel    neutral_flux   '
   write( 10, '(5(1PE15.6))' ) ( xcb(i), Gamma_n(i)*B_field_cb(i),Gamma_mom(i)*B_field_cb(i),q_parallel(i)*B_field_cb(i),neutral_flux(i), i=0,Nx ) ! multiplied by B_field because the code uses normalized values
