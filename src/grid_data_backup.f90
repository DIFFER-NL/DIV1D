...
!	if( case_single_null ) then   
   ...
   subroutine cc2cb(v,v_cb) !,delta_x,delta_xcb )
	   ...
	   sintheta_cb = sintheta
	     ...
	     open(1, file = 'sintheta.dat', status = 'old')
        	do i = 1,Nx
             ...
             close(1)
	     call cc2cb(sintheta_cc,sintheta_cb) 	     
	   endif
...
!<<<<<<< HEAD
          ...
          ! A_int = sol_width_pol_cb * 2.0d+0*pi*R_cb !(Is this correct?) 
	   A_int = volumes/delta_xcb ! this should always use the volumes from the plasma balance to stay consistent in domains
	   !A_int = sol_width_pol * 2.0d+0*pi*R_cc*sintheta_cc! no it should be multiplied with sintheta for the plasma equations
    	   !write(*,*) 'volumes' , size(volumes)
!	   A_wet(1) = volumes(1)/delta_xcb(1)
!	   A_wet(2) = volumes(Nx)/delta_xcb(Nx)
!=======
   	   !write(*,*) 'volumes' , size(volumes)
	   ...
	   !A_wet(1) = volumes(1)/delta_xcb(1)
	   !A_wet(2) = volumes(Nx)/delta_xcb(Nx)
	   ! with flux expansion on the above gives an error
	   !A_wet(1) = sol_width_pol_cb(0)*R_cb(0)*2.0d0*pi*sintheta_cb(0)
	   !A_wet(2) = sol_width_pol_cb(Nx)*R_cb(Nx)*2.0d0*pi*sintheta_cb(Nx)
!>>>>>>> dyn24-debug
   end subroutine width_surface_volume

   subroutine initialize_gas_puff
