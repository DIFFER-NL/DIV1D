...
! If not, see <https://www.gnu.org/licenses>. 
   ...
   use physics_parameters, only : gamma, mass, Gamma_X, q_parX, Gamma_core, Q_core, energy_loss_ion, recycling,	atom_recycle_energy_fraction, &
				...
				sintheta, minimum_density, maximum_density, minimum_temperature,maximum_temperature, density_ramp_rate, wide_PFR, sigma_nb,&
				...
				prf_imp_con, D_neu, D_dneu, D_nb, D_mb, D_gas, D_rec, D_qpar_x, D_red_frc, D_Q_core, &
   ...
   use numerics_parameters, only : Nx, evolve_density, evolve_momentum, evolve_energy, evolve_neutral, 		&
				 ...
				 switch_density_source, switch_momentum_source, switch_energy_source, switch_energy_flux, switch_energy_compr,	switch_only_core_source_Q,	& 		 					 switch_neutral_source, switch_neutral_momentum_source, switch_molecule_source, 	&
...
contains
  ...
  subroutine nvt2ys( Nx, density, velocity, temperature, neutral, neutral_velocity, molecule, ys )
   ...
   !	y( 4*Nx+1, ..., 5*Nx )	   = Neutral momentum = mass * density * neutral_velocity / momentum_norm
	...
	!OPEN(UNIT=10, FILE='density.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
	!WRITE(UNIT=10, FMT=*) 'Density:', density
	!CLOSE(UNIT=10)
      !write(*,*) 'momentum_norm' , momentum_norm
      return
   ...
   !	y( 4*Nx+1, ..., 5*Nx )	   = Neutral momentum = mass * density * neutral_velocity / momentum_norm
	...
	! removes NaN as well as constraining solutions
        temperature = min(max(temperature,minimum_temperature), maximum_temperature )
	!density = min(max(density,minimum_density), maximum_density )
	!neutral = min(max(neutral,minimum_density), maximum_density )
  ...
  subroutine yr2nr(yr, extern_neutral_density, extern_molecule_density)
   ...
   ! the flux is returned at the grid boundaries i + 1/2 (internal only: i=1,...,Nx-1, BC's must be applied outside this routine)
	 ...
	 ! add switch that takes out the limiter, variable_cb = cc2cb(variable_cc) flux(i) = var_cb(i)*vel_cb(i)	
         endif
      enddo
   ...
   ! ew 01-03-2021: modified to take account of flux expansion
	...
	if (isnan(molecule(ix))) stop 'calc_flux: "molecule" is a NaN'
      ...
      call cc2cb(density	,density_cb)   
      call cc2cb(velocity	,velocity_cb)
      call cc2cb(temperature	,temperature_cb)
      !call cc2cb(neutral 	,neutral_cb)
      call cc2cb(neutral_velocity, neutral_velocity_cb)
      !call cc2cb(neutral 	,neutral_cb)
      !call cc2cb(molecule 	,molecule_cb)
      csound_cb = sqrt( 2.0d+0 * e_charge * max(temperature_cb,minimum_temperature) / mass )   

      !OPEN(UNIT=10, FILE='velocity_cb.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'velocity_cb:', velocity_cb
      !CLOSE(UNIT=10)

      !OPEN(UNIT=10, FILE='density_cb.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'density_cb:', density_cb
      !CLOSE(UNIT=10)
 
      !OPEN(UNIT=10, FILE='temperature_cb.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'temperature_cb:', temperature_cb
      !CLOSE(UNIT=10)

      !OPEN(UNIT=10, FILE='B_field_cb.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'B_field_cb:', B_field_cb
      !CLOSE(UNIT=10)
 ...
 !  end subroutine calculate_leakage_corrected_Gamma_neutral_particles(flux,flow)
   ...
   ! this subroutine calculates the source terms of the discretized conservation equations
      ...
      real(wp) :: rate_cx(Nx), rate_ion(Nx), rate_exc(Nx), rate_rec(Nx), rate_ree(Nx), rate_imp(Nx), impurity_radiation_values(Nx), rate_mom(Nx), mol_density(Nx), rate_mom_m(Nx), rate_cx_m(Nx), &
	...
	!write(*,*) 'internal_istep_rhs', internal_istep_rhs
	 ...
	 impurity_radiation_values(ix) = impurity_radiation(temperature(ix),internal_istep_rhs,ix)
	...
	!write(*,*)'Hm_MAD', rate_MAD_Hmin(400)
      !OPEN(UNIT=10, FILE='impurity_radiation.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'impurity_radiation:', impurity_radiation_values
      !CLOSE(UNIT=10)

      !OPEN(UNIT=10, FILE='D_imp_con.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'D_imp_con:', D_imp_con
      !CLOSE(UNIT=10)
      ...
      !OPEN(UNIT=10, FILE='temperature.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      ...
      !OPEN(UNIT=10, FILE='prf_imp_con.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'prf_imp_con:', prf_imp_con
      !CLOSE(UNIT=10)
	...
	! neutral energy also adds to the ionization term  
      ...
      !OPEN(UNIT=10, FILE='Source_Q_1.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
      ...
      !OPEN(UNIT=10, FILE='Source_Q_1.1.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
      ...
      !OPEN(UNIT=10, FILE='Source_Q_1.2.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
      !write(194,*) 'D_imp_con:' , D_imp_con

      !OPEN(UNIT=10, FILE='rate_imp.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'rate_imp:', rate_imp
      !CLOSE(UNIT=10)
      ...
      !OPEN(UNIT=10, FILE='Source_Q_1.3.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
      ! Add contributions from H2plus and Hmin
      Source_Q = Source_Q + Q_dr + Q_di + Q_diss_H2plus + Q_cx_Hmin + Q_ion_Hmin

      !OPEN(UNIT=10, FILE='Source_Q_1.4.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
      ...
      !OPEN(UNIT=10, FILE='Source_Q_2.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
      !CLOSE(UNIT=10)
       !write(*,*) 'Stop 6.4'
	...
	if( evolve_core .eq. 1 ) then
	  ...
	  !write(*,*) 'DGamma'
       ...
       !write(*,*) 'Stop 6.5.1', SIZE(I_core_source_profile_n)
	...
	!write(*,*) 'Source_Q = 0'
       ...
       !write(*,*) 'Stop 6.5.2', SIZE(volumes)
       !write(*,*) 'Stop 6.5.2.1', i_Xpoint,L_core_SOL
	!write(*,*) '666 source_n:', Source_n
       ...
       !write(*,*) 'I_core_profile_n' ,I_core_source_profile_n
       !write(*,*) 'Stop 6.5.3'
	...
	!write(*,*) 'sol2extern_ion_flux =' , sol2extern_ion_flux
	...
	!write(*,*) '682 source_n:', Source_n
       ...
       !OPEN(UNIT=10, FILE='Source_Q.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
       !WRITE(UNIT=10, FMT=*) 'Source_Q:', Source_Q
       !CLOSE(UNIT=10)


       !OPEN(UNIT=10, FILE='I_core_source_profile_n_standalone.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
       !WRITE(UNIT=10, FMT=*) 'I_core_source_profile_n:', I_core_source_profile_n
       !CLOSE(UNIT=10)
	if( detect_nan ) then	
	do ix = 1,Nx
   ...
   !write(*,*) , core2sol_flux, core_sol_neutral_ex
   ! molecules do also shine through but with different factor
   !WRITE(120,*) 'core_sol_neutral_ex', core_sol_neutral_ex
...
!	extern_molecule_density = D_mb(1:5,internal_istep_rhs)	
 !     endif
	...
	!write(*,*) 'Temperature = ', temperature
       ...
       !write(120,*) 'pump_rate_n' , pump_rate_n
       !write(121,*) 'pump_rate_m' , pump_rate_m
       !write(122,*) 'puff_rate_n' , D_neutral_puff
       !write(123,*) 'puff_rate_mol' , D_molecule_puff
        ! call cc2cb(density	,density_cb)   
        call cc2cb(velocity	,velocity_cb)
        call cc2cb(temperature	,temperature_cb)
       !write(*,*) 'Stop 13'
	...
	! add the heat flux term including all boundaries
	 ...
	 !write(194,*) 'flux_source_neutral:' , flux_source_neutral
      ! --------------------------------------------- ydot for the neutral momentum equation ------------------------------------------
		...
		!    ydot(4*Nx+ix) = ydot(4*Nx+ix) - (Gamma_mom_neutral(ix)-Gamma_mom_neutral(ix-1))/delta_xcb(ix)   ! ew 01-03-2021:
	      !enddo
		!write(194,*) 'flux_source_neutral_momentum' , flux_source_neutral_momentum
	...
	!------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		!OPEN(UNIT=10, FILE='neutral.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
     		!WRITE(UNIT=10, FMT=*) 'neutral:', neutral
      		!CLOSE(UNIT=10)
	        !write(194,*) 'ydot_neutral:' , ydot(3*Nx+1:4*Nx)
		!write(195,*) 'ydot_neutral_mom:' , ydot(4*Nx+1:5*Nx)
		!OPEN(UNIT=10, FILE='molecule.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
      		!WRITE(UNIT=10, FMT=*) 'Molecule:', molecule
      		!CLOSE(UNIT=10)
		!write(*,*) 'energy_norm:' , energy_norm
		...
		!OPEN(UNIT=10, FILE='ydot_neutral.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
     		!WRITE(UNIT=10, FMT=*) 'ydot:', ydot(3*Nx+1:4*Nx)
      		!CLOSE(UNIT=10)

		!OPEN(UNIT=10, FILE='ydot_density.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
     		!WRITE(UNIT=10, FMT=*) 'ydot:', ydot(1:Nx)
      		!CLOSE(UNIT=10)

		!OPEN(UNIT=10, FILE='ydot_velocity.txt', STATUS='OLD', ACTION='WRITE', FORM='FORMATTED', POSITION = 'APPEND')
     		!WRITE(UNIT=10, FMT=*) 'ydot:', ydot(Nx+1:2*Nx)
      		!CLOSE(UNIT=10)
