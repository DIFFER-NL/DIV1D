...
! when the div1d_physics namelist is read the normalizations are defined
   ...
   real( wp ) :: minimum_density        = 1.0d+12      ! densities are not allowed to become smaller than this value [/m^3]
   ...
   real( wp ) :: maximum_temperature    = 1000 
...
!    do z = 1,5
   ...
   subroutine extern_read_physics_parameters(floatinphys, intinphys) !, strinphys, loginphys)
	  ...
	  core_ionization_fraction = floatinphys(75)
	  ato_res_asy = 	     floatinphys(76)
	  mol_res_asy = 	     floatinphys(77)
	  wall_association_probability = floatinphys(78)
	  ...
	  puff_rate_neutral(1:5)= floatinphys(120:124) ! Puff rate of neutral atoms
	  puff_rate_molecule(1:5)= floatinphys(125:129) ! puff rate of molecules
	  ...
	  wide_PFR		= intinphys(8)	      ! switch to extend PFR neutral volume to to X-point (0 means from target to baffle, 1 from target to X-point
          density_norm = 1.0d+19   !initial_n
          temperature_norm = 1.0e+0
          velocity_norm = 1e4
          momentum_norm = mass * density_norm * velocity_norm
          energy_norm = density_norm * e_charge * temperature_norm
